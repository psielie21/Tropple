...
const idPlayerLookup = {};
const playersConnLookup = {};
const getId = () => {
    let id = 100;
    const stepUp = () => {
        id++;
...
const shuffleAndDistributeDisks = () => {
    const disks = (0, disks_1.getDiskArray)();
...
// Base Connection
const handleKnownPlayer = (conn, currId) => {
...
const handleNewPlayer = (conn, currId) => {
    sendId(conn, currId);
};
const removePlayer = (conn) => {
    const player = players.find((player) => playersConnLookup[player] == conn);
    if (player) {
...
const sendId = (conn, id) => {
    conn.send("CONNECTION ID " + id);
};
// Game Lobby
const gameLobbyConnect = (args, conn, currId) => {
...
const gameLobbyStart = (args, conn, currId) => {
...
const updatePlayers = () => {
    ...
    players.forEach((player) => {
        ...
        if (players.length > 0 && player == players[0]) {
            ...
            conn.sendUTF("GAME_LOBBY RECEIVE_PLAYERS_SUCCESS NON-HOST " + playersString);
        }
    });
};
const startGame = () => {
    ...
    players.forEach((player) => {
        ...
        console.log("Sending " + (disk === null || disk === void 0 ? void 0 : disk.toString()) + " to player " + player);
        if (disk) {
...
// Game Screen
const gameScreenFoundPair = (args, conn, currId) => {
    if (args[0] == "IN_GAME" && args[1] == "SEND_PAIR") {
        ...
        console.log("PlayerDisk: " + playerDiskLookup[player][len - 1].getSymbolIds());
        if (playerDiskLookup[player][len - 1].includes(index) &&
...
const sendPlayerDisk = (player, disk) => {
    const conn = playersConnLookup[player];
    conn.sendUTF("IN_GAME RECEIVE_PLAYER_DISK " + disk.toString());
};
const sendCenterDisk = (disk) => {
    players.forEach((player) => {
        const conn = playersConnLookup[player];
        conn.sendUTF("IN_GAME RECEIVE_CENTER_DISK " + disk.toString());
    });
};
const sendPlayerPercentage = (updatedPlayer, percentage) => {
    players.forEach((player) => {
        const conn = playersConnLookup[player];
        conn.sendUTF("IN_GAME RECEIVE_PLAYER_PERCENTAGE " +
            updatedPlayer +
            " " +
            percentage.toFixed(2));
    });
};
const sendVictory = (player) => {
